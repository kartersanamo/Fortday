...
public final class GhostPreviewService {
    ...
    public void renderPreview(Player player) {
        ...
        // Reset blocks no longer part of the preview to their real state.
    ...
    private Set<BlockPosition> computePreview(Anchor anchor, BuildPieceType piece, float yaw) {
        ...
        switch (piece) {
            case WALL -> {
                // 1x3 wall in a plane; orientation follows face/yaw.
            ...
            case CONE -> {
                // Cone approximation: 3x3 base + 1 center above.
                for (int dx = -1; dx <= 1; dx++) {
                    for (int dz = -1; dz <= 1; dz++) {
                        out.add(new BlockPosition(x + dx, y, z + dz));
                    }
            ...
            case RAMP -> {
                // 3-step ramp oriented by horizontal face.
